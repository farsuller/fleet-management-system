...
object SpatialFunctions {
    ...
    fun distance(
        geom1: Expression<*>,
        geom2: Expression<*>,
    ): Function<Double> = CustomFunction("ST_Distance", DoubleColumnType(), geom1, geom2)
    ...
    fun lineLocatePoint(
        line: Expression<*>,
        point: Expression<*>,
    ): Function<Double> = CustomFunction("ST_LineLocatePoint", DoubleColumnType(), line, point)

    /** ST_LineInterpolatePoint: Returns a point interpolated along a line. */
    fun lineInterpolatePoint(
        line: Expression<*>,
        fraction: Expression<Double>,
    ): Function<PGgeometry> = CustomFunction("ST_LineInterpolatePoint", PostGISColumnType(), line, fraction)

    /** ST_Contains: Returns true if geometry A contains geometry B. */
    fun contains(
        geomA: Expression<*>,
        geomB: Expression<*>,
    ): Function<Boolean> = CustomFunction("ST_Contains", BooleanColumnType(), geomA, geomB)

    /** ST_AsText: Returns WKT representation of a geometry (e.g. LINESTRING(lng lat, ...)) */
    fun asText(geom: Expression<*>): Function<String> = CustomFunction("ST_AsText", VarCharColumnType(), geom)
}
