...
class ListIncidentsUseCase(
    ...
    suspend fun getActiveByVehicle(vehicleId: String): List<VehicleIncident> =
        repository.findActiveIncidentsByVehicleId(VehicleId(vehicleId))

    suspend fun getAllByVehicle(vehicleId: String): List<VehicleIncident> = repository.findIncidentsByVehicleId(VehicleId(vehicleId))

    suspend fun getByJob(jobId: String): List<VehicleIncident> = repository.findIncidentsByJobId(MaintenanceJobId(jobId))

    suspend fun getAll(status: IncidentStatus? = null): List<VehicleIncident> = repository.findAllIncidents(status)
}
