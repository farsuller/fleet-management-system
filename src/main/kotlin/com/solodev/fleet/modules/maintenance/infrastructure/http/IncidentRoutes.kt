    ...
    authenticate("auth-jwt") {
        route("/v1/vehicles") {
            ...
            post("/{plate}/incidents") {
                val plate =
                    call.parameters["plate"] ?: return@post call.respond(
                ...
                try {
                    ...
                    reportIncidentUseCase(
                        ...
                        call.respond(
                            ...
                            ApiResponse.success(IncidentResponse.fromDomain(incident, plate), call.requestId),
                        )
                    }.onFailure { e ->
                        call.respond(
                            HttpStatusCode.BadRequest,
                            ApiResponse.error("REPORT_FAILED", e.message ?: "Failed to report incident", call.requestId),
                        )
                    }
                } catch (e: Exception) {
                    call.respond(
                        HttpStatusCode.BadRequest,
                        ApiResponse.error("INVALID_REQUEST", e.message ?: "Invalid request", call.requestId),
                    )
                }
            }
        ...
        route("/v1/incidents") {
            get {
                ...
                val status =
                    statusParam?.let { s ->
                        try {
                            IncidentStatus.valueOf(s.uppercase())
                        } catch (e: IllegalArgumentException) {
