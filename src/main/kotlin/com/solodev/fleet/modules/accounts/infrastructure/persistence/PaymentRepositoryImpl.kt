...
class PaymentRepositoryImpl : PaymentRepository {
    ...
    override suspend fun findByIds(ids: List<UUID>): List<Payment> =
        dbQuery {
            PaymentsTable
                .selectAll()
    ...
    override suspend fun findByDriverId(driverId: UUID): List<Payment> =
        dbQuery {
            PaymentsTable.selectAll().where { PaymentsTable.driverId eq driverId }.map { toDomain(it) }
        }

    override suspend fun findPendingByDriverId(driverId: UUID): List<Payment> =
        dbQuery {
            PaymentsTable
                .selectAll()
