...
class RentalRepositoryImpl : RentalRepository {
    ...
    override suspend fun findByRentalNumber(rentalNumber: String): Rental? =
        dbQuery {
            RentalsTable
                .selectAll()
    ...
    override suspend fun findByCustomerId(customerId: CustomerId): List<Rental> =
        dbQuery {
            RentalsTable
                .selectAll()
    ...
    override suspend fun findByVehicleId(vehicleId: VehicleId): List<Rental> =
        dbQuery {
            RentalsTable
                .selectAll()
    ...
    override suspend fun findByStatus(status: RentalStatus): List<Rental> =
        dbQuery {
            RentalsTable.selectAll().where { RentalsTable.status eq status.name }.map { it.toRental() }
        }

    override suspend fun deleteById(id: RentalId): Boolean =
        dbQuery {
