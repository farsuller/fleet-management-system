...
class VehicleRepositoryImpl : VehicleRepository {
    ...
    override suspend fun findByPlateNumber(plateNumber: String): Vehicle? = dbQuery {
        VehiclesTable.select { VehiclesTable.plateNumber eq plateNumber }
                .map { it.toVehicle() }
                .singleOrNull()
    }
    ...
    suspend fun getOdometerHistory(vehicleId: VehicleId): List<OdometerReading> = dbQuery {
        OdometerReadingsTable.select {
            OdometerReadingsTable.vehicleId eq UUID.fromString(vehicleId.value)
        }
