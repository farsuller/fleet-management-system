...
class VehicleRepositoryImpl(
    private val cacheManager: RedisCacheManager? = null
) : VehicleRepository {
    ...
    override suspend fun save(vehicle: Vehicle): Vehicle = dbQuery {
        ...
        if (exists) {
            ...
            val updatedCount = VehiclesTable.update({
                (VehiclesTable.id eq vehicleUuid) and (VehiclesTable.version eq vehicle.version)
            }) {
                it[vin] = vehicle.vin
    ...
    override suspend fun findAll(params: PaginationParams): Pair<List<Vehicle>, Long> = dbQuery {
        ...
        val items = query.orderBy(VehiclesTable.id to SortOrder.ASC).limit(params.limit).map { it.toVehicle() }

        Pair(items, totalCount)
    }

    override suspend fun deleteById(id: VehicleId): Boolean = dbQuery {
        val deletedCount = VehiclesTable.deleteWhere { VehiclesTable.id eq UUID.fromString(id.value) }
        deletedCount > 0
    }
