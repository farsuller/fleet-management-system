...
-- Chart of accounts: Define all accounts in the system (Idempotent)
...
-- Ledger entries: Double-entry bookkeeping journal entries (Idempotent)
...
-- Ledger entry lines: Individual debit/credit lines (Idempotent)
...
-- Invoices table: Track customer invoices (Idempotent)
...
-- Payments table: Track all payments (Idempotent)
...
-- Insert default chart of accounts (Idempotent)
...
CREATE INDEX IF NOT EXISTS idx_ledger_entries_external_reference ON ledger_entries(external_reference);
CREATE INDEX IF NOT EXISTS idx_ledger_entries_entry_date ON ledger_entries(entry_date);
CREATE INDEX IF NOT EXISTS idx_ledger_entry_lines_entry_id ON ledger_entry_lines(entry_id);
...
-- Note: This trigger is deferred to allow all lines to be inserted before validation (Idempotent)
DROP TRIGGER IF EXISTS validate_ledger_balance ON ledger_entry_lines;
