...
class UpdateVehicleLocationUseCaseTest {
    ...
    @Test
    fun `should stop execution when reception is disabled`() =
        runBlocking {
            // Arrange
    ...
    @Test
    fun `should process and map all sensor fields when enabled`() =
        runBlocking {
            // Arrange
    ...
    @Test
    fun `should mark vehicle as OFF_ROUTE when distance exceeds 100m`(): Unit =
        runBlocking {
            // Arrange
            val command = createDummyCommand()
            coEvery { receptionService.isReceptionEnabled() } returns true
    ...
    @Test
    fun `should mark vehicle as IDLE when speed is low`(): Unit =
        runBlocking {
            // Arrange
    ...
    @Test
    fun `should throw exception when routeId is missing`(): Unit =
        runBlocking {
            // Arrange
